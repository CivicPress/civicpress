...
> **Current state — 2026-08-08 (v0.3.1).** CivicPress is a **working alpha**
> civic-records platform (v0.3.1, MIT). This document is the honest answer to
> _"is it ready?"_ and is verified feature-by-feature against the code (full
...
municipal procurement standards; expect breaking changes through v0.4.x.**
...
| Storage                          | **Partial** | Local FS provider works and is tested. S3 / GCS / Azure provider ops + init are covered against a **mocked SDK boundary**, plus an opt-in **live S3** integration test (v0.3.0, `CIVIC_TEST_LIVE_S3=1`, against minio). Both the API and CLI apply `storage.yml` (provider selection + `global.*` tuning).                                                                                                                                                                                               |
