...
- **BB-HW-010** (token in `?token=` query param) → **assessed, deferred — not a quick win.** The token is baked into the endpoint URL in two places (`main.py` + the re-enrollment path) and re-extracted into the `Authorization` header at connect time; dropping the URL form cleanly is a multi-site auth-flow refactor and is risky without a runnable test suite (hardware-repo deps not installed in the dev VM). Stays open for a focused security pass.
