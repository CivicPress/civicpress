...
| BB-HW-006 | `closed-with-commit-SHA` | 2026-06-23 | Phase 4 W1d (gate cleared once BB-HW-013 enrollment landed). `websocket_client.py` had 3–4 overlapping disconnect→reconnect paths — receive-loop failure, the health monitor, re-enrollment, and the reconnect loop's own self-restart — funnelling through a patchwork of `connected`/`_connecting`/`running`/`state` flags + lock-held re-checks that drifted (the health monitor even had an "inconsistent state" check for that drift). Extracted the connection lifecycle into one `ReconnectionManager` over `ConnectionState` (`connector/reconnection.py`, HW commit `1c5606d`): one state machine, one backoff (`5/10/20/40/60/60`, unchanged), one reconnect task, one trigger (`request_reconnect`) every path now calls; the socket-open + message-pump stay in the client as an injected `connect_fn`. The client's status flags became a *projection* of the manager's single authoritative state (one writer), so the ~112 readers + `is_connected()`/`get_state()` can't drift. The 6 W1d characterization tests (`df2a006`) were migrated to drive the manager with **identical assertions** (the safety net survives the extraction); +11 new manager contract tests. `websocket_client.py` 1380→1124 LoC (1,432 at audit time); reconnect logic now in the 212-LoC `reconnection.py`. Full HW suite 313 passed / 7 skipped; pushed to the private backup. Closes W1d → **Phase 4 W1 complete (BB-HW-001/004/005/006)**. |
