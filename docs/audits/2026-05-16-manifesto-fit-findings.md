...
- **BB-HW-015** (version/status claims disagree across `pyproject.toml` 0.1.0 / CHANGELOG `[0.1.0] - TBD` / `agent/goals.md` "core implementation complete") → **held for a maintainer status call.** Reconciling needs a real decision on the project's maturity; guessing it risks creating a *new* false claim — the opposite of this audit's intent. The sources also partially drifted since May.
