      ...
      withCli<[any]>(
        {
          ...
          errorMessage: 'Failed to delete test users',
          errorCode: 'DELETE_TEST_USERS_FAILED',
          details: (error) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
          }),
        },
      ...
      withCli<[any, any]>(
        {
          ...
          details: (error, username: any) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
            username,
          }),
        },
        async ({ globalOptions }, username, options) => {
      ...
      withCli<[any, any]>(
        {
          ...
          errorCode: 'REQUEST_EMAIL_CHANGE_ERROR',
          details: (error, username: any) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
            username,
          }),
  ...
  cli
    ...
    .action(
      withCli<[any, any]>(
        ...
        async ({ globalOptions }, token, _options) => {
          // Get configuration from central config
          const { CentralConfigManager } = await import('@civicpress/core');
          const dataDir = CentralConfigManager.getDataDir();
          const dbConfig = CentralConfigManager.getDatabaseConfig();
      ...
      withCli<[any, any]>(
        {
          ...
          errorMessage: 'Error cancelling email change',
          errorCode: 'CANCEL_EMAIL_CHANGE_ERROR',
          details: (error, username: any) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
            username,
          }),
      ...
      withCli<[any, any]>(
        {
          ...
          errorMessage: 'Error getting security info',
          errorCode: 'GET_SECURITY_INFO_ERROR',
          details: (error, username: any) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
            username,
          }),
