...
export default function setupStorageCommand(cli: CAC) {
  cli
    ...
    .action(async (options: any) => {
      // If --help is present, let CAC handle it and exit 0. This stays
      // OUTSIDE withCli deliberately: the wrapper opens an operation span
      // as its first act, and `--help` must not emit one.
      if (options.help) {
        return;
      }
      return withCli<[any]>(
        {
          ...
          errorMessage: 'Error managing storage configuration',
      ...
      return withCli<[any]>(
        {
          ...
          errorMessage: 'Error listing files',
      ...
      return withCli<[any]>(
        {
          ...
          errorMessage: 'Error adding storage folder',
          errorCode: 'ADD_FOLDER_ERROR',
          details: (error) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
          }),
        },
        async ({ globalOptions }, options: any) => {
          ...
          try {
            ...
            config.folders[options.name] = {
              ...
              description:
                options.description || `Storage folder: ${options.name}`,
            };
      ...
      return withCli<[any]>(
        {
          ...
          errorMessage: 'Error updating storage folder',
          errorCode: 'UPDATE_FOLDER_ERROR',
          details: (error) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
          }),
        },
      ...
      return withCli<[any]>(
        {
          ...
          errorMessage: 'Error removing storage folder',
          errorCode: 'REMOVE_FOLDER_ERROR',
          details: (error) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
          }),
        },
      ...
      return withCli<[any]>(
        {
          ...
          errorMessage: 'Error getting file information',
          errorCode: 'GET_FILE_INFO_ERROR',
          details: (error) => ({
            error: error instanceof Error ? error.message : 'Unknown error',
          }),
        },
