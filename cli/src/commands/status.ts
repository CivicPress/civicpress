          ...
          try {
            ...
            try {
              const configStatuses =
                coreMod.CentralConfigManager.getRecordStatusKeys();
            ...
            if (currentStatus === newStatus) {
              cliWarn(
                `Record "${recordName}" is already ${newStatus}`,
                'status'
              );
              process.exit(1);
            }
            ...
            if (isCompleteDryRun) {
              ...
              // Commit the change
              const git = new (await import('@civicpress/core')).GitEngine(
                dataDir
              );
            ...
            // Failure audit needs `recordName`; re-thrown so withCli emits
            // STATUS_CHANGE_FAILED and exits.
            await audit.log({
              source: 'cli',
              action: 'record_status_change',
