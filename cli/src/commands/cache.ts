...
export function registerCacheCommand(cli: CAC): void {
  cli
    ...
    .action(
      withCli<[{ json?: boolean; name?: string }]>(
        {
          operation: 'cache:metrics',
          errorMessage: 'Error getting cache metrics',
          errorCode: 'CACHE_METRICS_FAILED',
        },
        async (_ctx, options: { json?: boolean; name?: string }) => {
          ...
          if (options.name) {
            ...
            if (options.json) {
              ...
              cliInfo(
                `Total Caches: ${Object.keys(globalStats.caches).length}`
              );
              cliInfo(
  ...
  cli
    ...
    .option('--json', 'Output as JSON')
    .action(
      withCli<[{ json?: boolean }]>(
        {
          operation: 'cache:health',
          errorMessage: 'Error getting cache health',
          errorCode: 'CACHE_HEALTH_FAILED',
        },
        async (_ctx, options: { json?: boolean }) => {
          const config = await loadConfig();
          if (!config) {
            cliError(
              'No CivicPress configuration found. Run "civic init" first.'
      ...
      withCli<[{ json?: boolean }]>(
        {
          ...
          errorMessage: 'Error listing caches',
          errorCode: 'CACHE_LIST_FAILED',
        },
        async (_ctx, options: { json?: boolean }) => {
          const config = await loadConfig();
          if (!config) {
