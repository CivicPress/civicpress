...
describe('CLI History Command', () => {
  ...
  it('should show history for a record (manual test)', async () => {
    createMockRecord(
      context.testDir,
      'resolution',
      'history-me',
    ...
    execSync('git add resolution/history-me/', {
      cwd: join(context.testDir, 'data'),
    });
    execSync('git commit -m "Add history-me"', {
      cwd: join(context.testDir, 'data'),
    });
    const result = await runCivicCommand(
      'history resolution/history-me',
