...
const pageTitle = computed(() => {
    return user.value ? `View ${user.value.name || user.value.username}` : 'View User';
});
                ...
                <UCard v-else>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold">{{ user?.name || user?.username }}</h3>
                                <p class="text-sm text-gray-600 dark:text-gray-400">@{{ user?.username }}</p>
                                <UBadge
                                    :color="user?.role === 'admin' ? 'error' : user?.role === 'clerk' ? 'primary' : 'neutral'"
                                    variant="soft">
                                    {{ user?.role }}
                                </UBadge>
                            </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Full
                                        Name</label>
                                    <p class="text-sm text-gray-900 dark:text-gray-100">
                                        {{ user?.name || 'No name provided' }}</p>
                                </div>
                            </div>
                            <div class="space-y-4">
                                <div>
                                    <label
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Created</label>
                                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ user?.created_at ? new
                                        Date(user.created_at).toLocaleDateString() : 'Unknown' }}</p>
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Last
                                        Updated</label>
                                    <p class="text-sm text-gray-900 dark:text-gray-100">{{ user?.updated_at ? new
                                        Date(user.updated_at).toLocaleDateString() : 'Unknown' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
