<template>
    <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <UFormField label="Username"
            :description="isEditing ? 'Username cannot be changed after creation' : 'Choose a unique username for this user'"
            :hint="isEditing ? 'Read-only field' : 'Must be unique across the system'"
            :help-text="isEditing ? 'Username is permanent and cannot be modified' : 'Use lowercase letters, numbers, and hyphens only'">
            <UInput v-model="form.username" :placeholder="isEditing ? user?.username : 'Enter username'"
                :disabled="isEditing" :error="formErrors.username" required />
        </UFormField>

        <!-- Email Field -->
        <UFormField label="Email Address" description="Primary contact email for this user"
            hint="Used for login and notifications" help-text="Must be a valid email address">
            <UInput v-model="form.email" type="email" placeholder="Enter email address" :error="formErrors.email"
                required />
        </UFormField>

        <!-- Full Name Field -->
        <UFormField label="Full Name" description="User's full name (optional)" hint="Will use username if not provided"
            help-text="Display name shown throughout the system">
            <UInput v-model="form.name" placeholder="Enter full name (optional)" :error="formErrors.name" />
        </UFormField>

        <!-- Role Field -->
        <UFormField label="Role" description="User's role and permissions"
            hint="Determines access level and capabilities" help-text="Role cannot be changed after creation">
            <USelectMenu v-model="roleOptionsSelect" :items="roleOptions" placeholder="Select a role"
                :error="formErrors.role" :loading="rolesLoading" required class="w-full sm:w-48" />
        ...
        <template v-if="!isEditing || showPasswordFields">
            <UFormField label="Password"
                :description="isEditing ? 'Enter new password to change' : 'Create a secure password'"
                ...
                <div class="flex space-x-2">
                    <UInput v-model="form.password" :type="showPassword ? 'text' : 'password'"
                        placeholder="Enter password" :error="formErrors.password" class="flex-1" required />
                    <UButton type="button" icon="i-lucide-refresh-cw" color="neutral" variant="outline" size="sm"
                        @click="generatePassword" title="Generate secure password" />
                    <UButton type="button" :icon="showPassword ? 'i-lucide-eye-off' : 'i-lucide-eye'" color="neutral"
                        variant="outline" size="sm" @click="showPassword = !showPassword"
                ...
                <template #help>
                    <div v-if="form.password" class="space-y-2">
                        ...
                        <div class="text-xs text-gray-500">
                            Password must be at least 8 characters with uppercase, lowercase, numbers, and special
                            characters
                        </div>
                    </div>
                </template>
            ...
            <UFormField label="Confirm Password" description="Re-enter the password to confirm"
                hint="Must match the password above" help-text="Passwords must match exactly">
                <UInput v-model="form.confirmPassword" :type="showPassword ? 'text' : 'password'"
                    placeholder="Confirm password" :error="formErrors.confirmPassword" required />
            </UFormField>
        </template>

        <!-- Change Password Button (for editing) -->
        <div v-if="isEditing && !showPasswordFields" class="pt-4">
            <UButton type="button" color="neutral" variant="outline" icon="i-lucide-key"
        ...
        <div class="flex justify-between items-center pt-6">
            ...
            <div v-if="isEditing && canDelete" class="flex-1">
                ...
                <UModal title="Delete User"
                    ...
                    <UButton type="button" color="error" variant="outline" icon="i-lucide-trash-2"
                        @click="showDeleteModal = true" :loading="deleting">
                        Delete User
                    </UButton>
            ...
            <div class="flex space-x-3 ml-auto">
                <UButton type="button" color="neutral" variant="outline" @click="$router.back()">
                    Cancel
                </UButton>
                <UButton type="submit" color="primary" :loading="saving" :disabled="!isFormValid">
...
onMounted(async () => {
    ...
    if (props.isEditing && props.user) {
        ...
        const userRole = roleOptions.value.find(role => role.value === props.user?.role);
        if (userRole) {
...
const isFormValid = computed(() => {
    ...
    const hasValidPassword = props.isEditing
        ? (!showPasswordFields.value || (form.password && form.password === form.confirmPassword))
