...
export class SessionController {
  ...
  constructor(
    ...
    // Get database service from deviceManager (via its internal model)
    const db = (deviceManager as any).db;
  ...
  async startSession(request: StartSessionRequest): Promise<BroadcastSession> {
    ...
    // Verify CivicPress session record exists
    ...
    if (room) {
      ...
      throw new Error(`Device room not found: device:${request.deviceId}`);
    }
  ...
  async stopSession(sessionId: string): Promise<BroadcastSession> {
    ...
    if (room) {
      ...
      throw new Error(`Device room not found: device:${session.deviceId}`);
    }
